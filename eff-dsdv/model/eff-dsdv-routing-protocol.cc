...
const uint32_t RoutingProtocol::EFFDSDV_PORT = 269;
...
struct DeferredRouteOutputTag : public Tag
{
  ...
  GetTypeId ()
  {
    static TypeId tid = TypeId ("ns3::eff-dsdv::DeferredRouteOutputTag").SetParent<Tag> ();
    return tid;
  }
...
{
  static TypeId tid = TypeId ("ns3::eff-dsdv::RoutingProtocol")
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No eff-dsdv interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
                             ...
                             ErrorCallback ecb)
{
  ...
  if (m_socketAddresses.empty ())
    {
      NS_LOG_DEBUG ("No eff-dsdv interfaces");
      return false;
    }
  NS_ASSERT (m_ipv4 != 0);
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid Eff-DSDV source address not found");
...
RoutingProtocol::RecvEffDsdv (Ptr<Socket> socket)
{
  ...
  for (; packetSize > 0; packetSize = packetSize - 12)
    {
      count = 0;
      EffDsdvHeader effDsdvHeader, tempEffDsdvHeader;
      packet->RemoveHeader (effDsdvHeader);
      NS_LOG_DEBUG ("Processing new update for " << effDsdvHeader.GetDst ());
      ...
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j
           != m_socketAddresses.end (); ++j)
        {
          ...
          if (effDsdvHeader.GetDst () == interface.GetLocal ())
            {
              if (effDsdvHeader.GetDstSeqno () % 2 == 1)
                {
                  NS_LOG_DEBUG ("Sent Eff-Dsdv update back to the same Destination, "
      ...
      NS_LOG_DEBUG ("Received a Eff-DSDV packet from "
                    << sender << " to " << receiver << ". Details are: Destination: " << effDsdvHeader.GetDst () << ", Seq No: "
                    << effDsdvHeader.GetDstSeqno () << ", HopCount: " << effDsdvHeader.GetHopCount ());
      RoutingTableEntry fwdTableEntry, advTableEntry;
      EventId event;
      bool permanentTableVerifier = m_routingTable.LookupRoute (effDsdvHeader.GetDst (),fwdTableEntry);
      if (permanentTableVerifier == false)
          ...
          if (effDsdvHeader.GetDstSeqno () % 2 != 1)
            {
              ...
              RoutingTableEntry newEntry (
                /*device=*/ dev, /*dst=*/
                effDsdvHeader.GetDst (), /*seqno=*/
                effDsdvHeader.GetDstSeqno (),
                /*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0),
                /*hops=*/ effDsdvHeader.GetHopCount (), /*next hop=*/
                sender, /*lifetime=*/
        ...
        {
          if (!m_advRoutingTable.LookupRoute (effDsdvHeader.GetDst (),advTableEntry))
            {
              ...
              m_advRoutingTable.LookupRoute (effDsdvHeader.GetDst (),advTableEntry);
            }
          if (effDsdvHeader.GetDstSeqno () % 2 != 1)
            {
              if (effDsdvHeader.GetDstSeqno () > advTableEntry.GetSeqNo ())
                {
                  // Received update with better seq number. Clear any old events that are running
                  if (m_advRoutingTable.ForceDeleteIpv4Event (effDsdvHeader.GetDst ()))
                    {
                  ...
                  if (effDsdvHeader.GetHopCount () != advTableEntry.GetHop ())
                    {
                      advTableEntry.SetSeqNo (effDsdvHeader.GetDstSeqno ());
                      ...
                      advTableEntry.SetHop (effDsdvHeader.GetHopCount ());
                      NS_LOG_DEBUG ("Received update with better sequence number and changed metric.Waiting for WST");
                      Time tempSettlingtime = GetSettlingTime (effDsdvHeader.GetDst ());
                      ...
                      m_advRoutingTable.AddIpv4Event (effDsdvHeader.GetDst (),event);
              ...
              else if (effDsdvHeader.GetDstSeqno () == advTableEntry.GetSeqNo ())
                {
                  if (effDsdvHeader.GetHopCount () < advTableEntry.GetHop ())
                    {
                      ...
                      m_advRoutingTable.ForceDeleteIpv4Event (effDsdvHeader.GetDst ());
                      advTableEntry.SetSeqNo (effDsdvHeader.GetDstSeqno ());
                      advTableEntry.SetLifeTime (Simulator::Now ());
                      advTableEntry.SetFlag (VALID);
                      advTableEntry.SetEntriesChanged (true);
                  ...
                  else
                    {
                       ...
                       * Discard that update.
                       */
                      if (!m_advRoutingTable.AnyRunningEvent (effDsdvHeader.GetDst ()))
                        {
                          ...
                          m_advRoutingTable.DeleteRoute (
                            effDsdvHeader.GetDst ());
                        }
                      NS_LOG_DEBUG ("Received update with same seq number and "
                                    "same/worst metric for, " << effDsdvHeader.GetDst () << ". Discarding the update.");
                    }
                }
              else
                {
                  ...
                  if (!m_advRoutingTable.AnyRunningEvent (effDsdvHeader.GetDst ()))
                    {
                      m_advRoutingTable.DeleteRoute (effDsdvHeader.GetDst ());
                    }
                  NS_LOG_DEBUG (effDsdvHeader.GetDst () << " : Received update with old seq number. Discarding the update.");
                }
            }
          else
            {
              NS_LOG_DEBUG ("Route with infinite metric received for "
                            << effDsdvHeader.GetDst () << " from " << sender);
              ...
              if (sender == advTableEntry.GetNextHop ())
                {
                  ...
                  m_routingTable.GetListOfDestinationWithNextHop (effDsdvHeader.GetDst (),dstsWithNextHopSrc);
                  m_routingTable.DeleteRoute (effDsdvHeader.GetDst ());
                  advTableEntry.SetSeqNo (effDsdvHeader.GetDstSeqno ());
                  advTableEntry.SetEntriesChanged (true);
                  m_advRoutingTable.Update (advTableEntry);
              ...
              else
                {
                  ...
                  NS_LOG_DEBUG (effDsdvHeader.GetDst () <<
      ...
      for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator i = allRoutes.begin (); i != allRoutes.end (); ++i)
        {
          ...
          if ((i->second.GetEntriesChanged () == true) && (!m_advRoutingTable.AnyRunningEvent (temp.GetDestination ())))
            {
              effDsdvHeader.SetDst (i->second.GetDestination ());
              effDsdvHeader.SetDstSeqno (i->second.GetSeqNo ());
              effDsdvHeader.SetHopCount (i->second.GetHop () + 1);
              temp.SetFlag (VALID);
              ...
              packet->AddHeader (effDsdvHeader);
      ...
      if (packet->GetSize () >= 12)
        {
          ...
          effDsdvHeader.SetDst (m_ipv4->GetAddress (1, 0).GetLocal ());
          effDsdvHeader.SetDstSeqno (temp2.GetSeqNo ());
          effDsdvHeader.SetHopCount (temp2.GetHop () + 1);
          ...
          socket->SendTo (packet, 0, InetSocketAddress (destination, EFFDSDV_PORT));
      ...
      for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator i = allRoutes.begin (); i != allRoutes.end (); ++i)
        {
          ...
          if (i->second.GetHop () == 0)
            {
              ...
              effDsdvHeader.SetDst (m_ipv4->GetAddress (1,0).GetLocal ());
              effDsdvHeader.SetDstSeqno (i->second.GetSeqNo () + 2);
              effDsdvHeader.SetHopCount (i->second.GetHop () + 1);
              m_routingTable.LookupRoute (m_ipv4->GetAddress (1,0).GetBroadcast (),ownEntry);
              ownEntry.SetSeqNo (effDsdvHeader.GetDstSeqno ());
          ...
          NS_LOG_DEBUG ("Forwarding details are, Destination: " << effDsdvHeader.GetDst ()
                                                                << ", SeqNo:" << effDsdvHeader.GetDstSeqno ()
                                                                << ", HopCount:" << effDsdvHeader.GetHopCount ()
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), EFFDSDV_PORT));
