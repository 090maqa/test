...
NS_LOG_COMPONENT_DEFINE ("EffDsdvManetExample");

class EffDsdvManetExample
{
public:
  EffDsdvManetExample ();
...
int main (int argc, char **argv)
{
  ...
  std::string CSVfileName = "EffDsdvManetExample.csv";
  ...
  cmd.AddValue ("CSVfileName", "The name of the CSV output file name[Default:EffDsdvManetExample.csv]", CSVfileName);
  cmd.Parse (argc, argv);
...
EffDsdvManetExample::EffDsdvManetExample ()
  : bytesTotal (0),
    packetsReceived (0)
{
}
...
void
EffDsdvManetExample::ReceivePacket (Ptr <Socket> socket)
{
...
void
EffDsdvManetExample::CheckThroughput ()
{
  ...
  out.close ();
  packetsReceived = 0;
  Simulator::Schedule (Seconds (1.0), &EffDsdvManetExample::CheckThroughput, this);
}

Ptr <Socket>
EffDsdvManetExample::SetupPacketReceive (Ipv4Address addr, Ptr <Node> node)
{
  ...
  sink->Bind (local);
  sink->SetRecvCallback (MakeCallback ( &EffDsdvManetExample::ReceivePacket, this));

  return sink;
}

void
EffDsdvManetExample::CaseRun (uint32_t nWifis, uint32_t nSinks, double totalTime, std::string rate,
                           ...
                           double dataStart, bool printRoutes, std::string CSVfileName)
{
  ...
  std::string tr_name = "Eff_Dsdv_Manet_" + t_nodes + "Nodes_" + sTotalTime + "SimTime";
...
void
EffDsdvManetExample::CreateNodes ()
{
...
void
EffDsdvManetExample::SetupMobility ()
{
...
void
EffDsdvManetExample::CreateDevices (std::string tr_name)
{
...
void
EffDsdvManetExample::InstallInternetStack (std::string tr_name)
{
  EffDsdvHelper effdsdv;
  effdsdv.Set ("PeriodicUpdateInterval", TimeValue (Seconds (m_periodicUpdateInterval)));
  effdsdv.Set ("SettlingTime", TimeValue (Seconds (m_settlingTime)));
  InternetStackHelper stack;
  stack.SetRoutingHelper (effdsdv); // has effect on the next Install ()
  stack.Install (nodes);
  ...
  if (m_printRoutes)
    {
      ...
      effdsdv.PrintRoutingTableAllAt (Seconds (m_periodicUpdateInterval), routingStream);
    }
}

void
EffDsdvManetExample::InstallApplications ()
{
